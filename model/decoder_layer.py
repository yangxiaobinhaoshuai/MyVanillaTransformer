...
# Allow running this file directly via `uv run python model/decoder_layer.py`.
...
class DecoderLayer(Module):
    ...
    input:
        ...
        tgt_mask:   bs, 1, seq_len_tgt, seq_len_tgt  # causal + padding mask
        src_mask:   bs, 1, 1,           seq_len_src  # encoder padding mask
    ...
    def __init__(
        ...
        #    target 序列内部的 self-attention，必须加 causal mask 防止看到未来。
        self.self_attn = MultiHeadAttention(d_model, n_heads, dropout)

        # 2. Cross-attention：
        #    query 来自 decoder 自己的 x，key/value 来自 encoder 的输出 enc_out。
        #    这是 decoder 读取 encoder 信息的地方。
        self.cross_attn = MultiHeadAttention(d_model, n_heads, dropout)

        # 3. Position-wise FFN，和 encoder 里完全一样。
    ...
    def forward(
        ...
        # 1. masked self-attention
        # q/k/v 都来自 x，用 tgt_mask 屏蔽未来位置 + padding。
        self_attn_out, self_attn_w = self.self_attn(x, x, x, tgt_mask)
        x = self.norm1(x + self.dropout1(self_attn_out))

        # 2. cross-attention
        # q 来自 decoder 当前的 x，k/v 来自 encoder 输出 enc_out。
        # src_mask 用来屏蔽 encoder 侧的 padding。
...
if __name__ == "__main__":
    ...
    # encoder 的最终输出（memory），cross-attention 的 k/v 来源。
    enc_out = torch.randn(bs, seq_len_src, d_model)

    # target 侧的 causal mask：下三角矩阵，屏蔽未来位置。
    # shape 要和 attention 分数对齐：(bs, 1, seq_len_tgt, seq_len_tgt)
    ...
    # encoder 侧 padding mask（这里假设没有 padding，全 1）。
    ...
    layer_out, layer_self_w, layer_cross_w = decoder_layer(
        x, enc_out, tgt_mask, src_mask
    )

    print("=== decoder layer check ===")
    print("decoder layer out shape:", layer_out.shape)
    ...
    print("=== decoder stack check ===")
    print("decoder out shape:", dec_out.shape)
    print("num layers:", len(all_self_w))
    print("layer 0 self attn_w shape:", all_self_w[0].shape)
    print("layer 0 cross attn_w shape:", all_cross_w[0].shape)
    ...
    assert all_cross_w[0].shape == (bs, n_heads, seq_len_tgt, seq_len_src)

    # 验证 causal mask 生效：第 0 个位置只能看自己，后面位置的注意力必须为 0。
